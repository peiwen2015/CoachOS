...
```text
releases/CoachOS-mRelay-Android-v0.2.0-debug.apk
```
...
5. Tap `存數據分頁`.
6. Switch to `間歇訓練` or `計圈`.
7. Tap `存第二頁並複製`.
8. Use `預覽` to inspect the payload, or `分享` to send it to another app.
9. Paste the clipboard content into ChatGPT, Claude, Gemini, or CoachOS.
