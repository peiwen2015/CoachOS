...
CoachOS v1.4.0 is the first released product version of `Workout Sequence Intelligence`.

## What This Release Means

This release moves CoachOS beyond single-activity interpretation and establishes the shipped `Workout Sequence Intelligence` product layer.
...
CoachOS v1.4.0 是 `Workout Sequence Intelligence` 第一次正式進入產品介面的已發布版本。
...
這個版本已經是可對外使用的基準版本。接下來如果要修文字、修邏輯、修判讀口徑，會往 `v1.4.1` 前進。

## English

CoachOS v1.4.0 is the first released version where `Workout Sequence Intelligence` becomes a product-facing layer.
...
`v1.4.0` is now the shipped baseline. Follow-up fixes and refinements should move into `v1.4.1`.
...
- `v1.4.0` is the first shipped `Workout Sequence Intelligence Product Layer` release
...
- Post-release polishing should continue in `v1.4.1`
